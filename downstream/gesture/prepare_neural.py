...
Prepare Denoised MAT Files for STCNet — Baseline Models
========================================================

與 prepare_denoised_mat_v11_2.py 完全相同的 pipeline，
唯一差異：MECG-E denoiser → waveform baseline model denoiser。

支援的 models（waveform-in / waveform-out）：
  - CNN_waveform
  - FCN
  - MSEMG
...
Baseline models 是純 waveform denoiser，不需要 OLA：
  Input:  [B, L=2000]  noisy_norm (same as MECG-E)
  Output: [B, L=2000]  denoised_norm

其餘 seg_lookup / QC index / canvas 邏輯與 v11_2 完全一致。
...
# ============================================================================
# Path Configuration  (match v11_2)
# ============================================================================
...
# ============================================================================
# QC Index + Segment Lookup  (identical to v11_2)
# ============================================================================
...
class BaselineDenoiser:
    """
    Wraps any waveform baseline model (CNN/FCN/MSEMG/TrustEMGNet).

    All baseline models share the same interface:
      Input:  [B, L]  float32
      Output: [B, L]  float32

    No OLA needed — models are fully causal/non-causal waveform nets
    that already handle the full segment length in one forward pass.

    Note: CNN_waveform internally splits 2000-sample input into
    10 × 200-sample chunks, so L must be exactly 2000 for that model.
    The pipeline uses seg_len_s=2.0 @ 1000 Hz → L=2000, so it works.
    """
    ...
    def __init__(self, model_name: str, model_path: str, device: str = "cuda"):
        ...
        # Import from baseline_models package
    ...
    def denoise_batch(self, noisy_batch: np.ndarray) -> np.ndarray:
        """
        Input:  noisy_batch [B, L] float32  (normalized domain ~[-1,1])
        Output: denoised    [B, L] float32
        """
        noisy_t = torch.from_numpy(noisy_batch).float().to(self.device)
        pred_t  = self.model(noisy_t)   # [B, L]
        return pred_t.cpu().numpy().astype(np.float32)


# ============================================================================
# Optional visualization  (identical to v11_2)
# ============================================================================

def _maybe_plot(vis_output_dir, rel, plot_id, baseline_seg,
                noisy_seg, denoised_seg, model_name, snr=None):
    ...
    axes[1].plot(baseline_seg[tail_start:], label="baseline", lw=1.0, color="steelblue")
    axes[1].plot(noisy_seg[tail_start:],    label="noisy",    lw=0.8, color="orange", alpha=0.7)
    axes[1].plot(denoised_seg[tail_start:], label="denoised", lw=1.0, color="crimson")
    axes[1].set_title("Last 300ms")
    axes[1].legend(loc="upper right", fontsize=7)
    axes[1].set_xlabel("samples (offset)")

    out_png = os.path.join(
        vis_output_dir, f"{os.path.basename(rel)}__{plot_id:02d}.png"
    )
    fig.tight_layout()
...
# ============================================================================
# Main Processing  (same flow as v11_2, MECGEDenoiser replaced)
# ============================================================================
...
def process_db2(
    ...
    pts = int(round(seg_len_s * target_fs))   # 2000

    stats = {
        "processed": 0, "skipped": 0, "errors": 0,
        "segs_placed": 0, "segs_missing": 0, "rms_warn": 0,
    }
    ...
    for mat_path in tqdm(mat_files, desc=f"Processing DB2 [{model_name}]"):
        ...
        # Baseline canvas: bandpass + resample (same as v11_2)
                    ...
                    if key in seg_lookup:
                        ...
                        if canvas_seg_start + pts <= N_1k:
                            baseline_seg = emg_1k_whole[
                                canvas_seg_start: canvas_seg_start + pts, ch
                            ].copy()
                            pending.append((
                                canvas_seg_start, ch,
                                entry["noisy"], entry["scale"],
        ...
        # Batch denoise
        if pending:
            noisy_norms    = np.stack([p[2] for p in pending], axis=0)   # [B, pts]
            ...
            for idx, (seg_start, ch, noisy_norm, scale, snr, baseline_seg, rel_local) \
                ...
                if vis_output_dir is not None and plots_done < int(vis_n_per_file):
                    _maybe_plot(
                        vis_output_dir, rel_local, plots_done,
                        baseline_seg, noisy_physical, denoised_physical,
                        model_name=model_name, snr=snr,
                    )
                    plots_done += 1
    ...
    if all_rms_ratios_denoised:
        ...
        print(f"\n  [振幅診斷] Denoised / Baseline RMS ratio:")
        print(f"    mean={rd.mean():.4f}  median={np.median(rd):.4f}  "
              f"std={rd.std():.4f}  min={rd.min():.4f}  max={rd.max():.4f}")
        print(f"  [振幅診斷] Noisy / Baseline RMS ratio:")
        print(f"    mean={rn.mean():.4f}  median={np.median(rn):.4f}  "
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare denoised .mat for STCNet — Baseline waveform models",
