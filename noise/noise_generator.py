...
1. Offline noise-library generation
   - Generates PLI, ECG, MOA, WGN, and colored-noise waveforms.
   - Applies the configured preprocessing filters to generated noise.
   - Saves each noise type into train/test noise-library directories.
   - Writes source manifests for ECG and MOA noise when applicable.

2. Online noise mixing for training
   - Loads a prebuilt noise library.
   - Samples one or more noise types for each clean sEMG segment.
   - Mixes selected noise components at a target total SNR.
   - Supports reproducible sampling through explicit random seeds.

WGN power allocation
--------------------
When k noise types are selected, total noise power is determined by the
requested total SNR:

    total_noise_power = clean_power / 10^(total_snr / 10)

By default, total noise power is divided equally across selected noise types.
For WGN, a minimum per-component SNR floor is enforced. If equal allocation
would make WGN stronger than this floor, WGN power is capped and the remaining
noise power is redistributed across the other selected noise types.

For k=1 with WGN below the floor, WGN is removed from the candidate pool because
...
def _get_bp_params(config):
    ...
    return (
        bool(bp.get("enabled", True)),
        float(bp.get("low_cutoff", 20.0)),
        float(bp.get("high_cutoff", 500.0)),
        int(bp.get("order", 4)),
    )
...
def _get_notch_params(config):
    ...
    return (
        bool(nc.get("enabled", True)),
        float(nc.get("freq_hz", 50.0)),
        float(nc.get("q", 30.0)),
    )
...
def _ecg_split_file(config):
    d = os.path.join(get_output_base(config), "noise")
    os.makedirs(d, exist_ok=True)
    return os.path.join(d, "ecg_split.json")


def get_or_create_ecg_test_ids(config, ecg_ids):
    sp = _ecg_split_file(config)
    cfg = config.get("noise", {}).get("generation", {}).get("ecg", {})

    cnt = int(cfg.get("test_record_count", 4))
    ...
    test_ids = rng.sample(list(ecg_ids), cnt)
    train_ids = [r for r in ecg_ids if r not in set(test_ids)]

    json.dump(
        {
            "seed": seed,
            "test_record_count": cnt,
            "test_ids": test_ids,
            "train_ids": train_ids,
        },
        open(sp, "w"),
...
class PLIGenerator(NoiseGenerator):
    def generate(self, count, config):
        ...
        for _ in range(count):
            ...
            if c.get("drift_enabled", True):
                f0 += random.uniform(
                    -float(c.get("drift_range", 0.3)),
                    float(c.get("drift_range", 0.3)),
                )

            H = int(c.get("harmonics", 5))
            alpha = float(c.get("amplitude_decay", 1.0))

            hs = (
                list(range(1, H + 1))
                if c.get("all_harmonics", True)
                else list(range(1, 2 * H, 2))[:H]
            )

            y = sum(
                (1 / k**alpha)
                * np.sin(2 * np.pi * k * f0 * t + 2 * np.pi * random.random())
                for k in hs
            )
...
class WGNGenerator(NoiseGenerator):
    def generate(self, count):
        return [
            ensure_length(self._bp(np.random.normal(0, 1, self.L)), self.L)
            for _ in range(count)
        ]
...
class ColorNoiseGenerator(NoiseGenerator):
    def generate(self, count, config):
        ...
        types = c.get("types", ["pink", "brown"])
        mode = str(c.get("sample_mode", "random")).lower()
        pa = float(c.get("pink_alpha", 1.0))
        ba = float(c.get("brown_alpha", 2.0))

        freqs = np.fft.rfftfreq(self.L, 1.0 / self.target_fs)
        freqs[0] = 1e-10
...
class ECGGenerator(NoiseGenerator):
    ...
    def __init__(self, ecg_root, target_fs, time_length, config):
        ...
        self.fs_src = 128
        self.seg_src_len = int(time_length * 128)
        self.notch_en, self.notch_freq, self.notch_q = _get_notch_params(config)

    def _read_128(self, rid, start):
        try:
            rec = wfdb.rdrecord(
                os.path.join(self.ecg_root, str(rid)),
                sampfrom=int(start),
                sampto=int(start + self.seg_src_len),
            )
    ...
    def _rlen(self, rid):
        try:
            return int(wfdb.rdheader(os.path.join(self.ecg_root, str(rid))).sig_len)
        except Exception:
            return None
    ...
    def _proc(self, x128):
        ...
        if self.notch_en:
            x = apply_notch_filter(x, self.target_fs, self.notch_freq, self.notch_q)

        return ensure_length(
            self._bp(apply_lowpass_filter(x, self.target_fs, 200.0)),
            self.L,
        )
    ...
    def _ok(x):
        s = x.std()
        return s > 1e-6 and np.abs(x).max() > 1e-6 and np.isfinite(s)
    ...
    def generate_cross_hour(self, ids, total, sph=1, hours=24, seed=None):
        ...
        for _ in range(total * 10):
            ...
            hs = h * int(3600 * self.fs_src)
            he = min(rl, (h + 1) * int(3600 * self.fs_src))

            if he - hs <= self.seg_src_len + 1:
                continue

            s = rng.randint(hs, he - self.seg_src_len)
            x128 = self._read_128(rid, s)
    ...
    def generate_random(self, ids, total, seed=None):
        ...
        for _ in range(total * 10):
            ...
            if not rl or rl <= self.seg_src_len + 1:
                continue

            s = rng.randint(0, rl - self.seg_src_len)
            x128 = self._read_128(rid, s)
...
class MOAGenerator(NoiseGenerator):
    ...
    def generate(self):
        ...
        mat_files = sorted(
            glob(os.path.join(self.moa_path, "**", "*.mat"), recursive=True)
        )
        ...
        for mp in mat_files:
            try:
                ...
                seg = (
                    ensure_length(x, self.L)
                    if x.size < self.L
                    else x[random.randint(0, x.size - self.L):][:self.L]
                )
...
def build_noise_library(config, mode):
    ...
    seed = int(config.get("project", {}).get("random_seed", 12345)) + (
        0 if is_train else 999
    )
    seed_everything(seed)

    src = config["paths"]["noise_sources"]
    fs = int(
        config["noise"]["generation"].get(
            "target_fs",
            config["preprocessing"]["segmentation"]["target_fs"],
        )
    )
    tlen = int(config["noise"]["generation"]["time_length"])
    counts = config["noise"]["generation"][f"{mode}_count"]

    print(f"\n{'=' * 80}\nBuild Noise Library ({mode})\n{'=' * 80}")
    print(
        f"WGN floor: {WGN_COMPONENT_SNR_MIN_DB} dB component SNR "
        f"(applied during mixing)"
    )

    def savedir(n):
        d = os.path.join(noise_dir, n)
        os.makedirs(d, exist_ok=True)
        return d

    print("\n[1/5] PLI")
    g = PLIGenerator(fs, tlen, config)
    lst = g.generate(int(counts.get("PLI", 0)), config)
    d = savedir("PLI")
    ...
    print("\n[2/5] WGN")
    g = WGNGenerator(fs, tlen, config)
    lst = g.generate(int(counts.get("WGN", 0)))
    d = savedir("WGN")
    ...
    g = ColorNoiseGenerator(fs, tlen, config)
    results = g.generate(int(counts.get("Color", 0)), config)
    d = savedir("Color")
    pc = 0
    bc = 0
    ...
    if os.path.exists(src["ecg"]):
        ...
        segs = int(
            ec.get(
                "train_segments" if is_train else "test_segments",
                24 if is_train else 4,
            )
        )

        if hourly:
            lst, man = ecg.generate_cross_hour(
                ids,
                segs,
                int(ec.get("segments_per_hour", 1)),
                int(ec.get("total_hours", 24)),
                seed,
            )
        else:
    ...
    for nt in ["PLI", "WGN", "Color", "ECG", "MOA"]:
        ...
        if os.path.isdir(nd):
            ...
            if nt == "Color":
                np_ = len(glob(os.path.join(nd, "*_pink_*.npy")))
                nb = len(glob(os.path.join(nd, "*_brown_*.npy")))
...
def _compute_noise_targets(
    ...
    - Redistribute remaining power to all non-WGN selected noise types.
    """
    k = len(selected)
...
class OnlineNoiseMixer:
    """
    Online noise mixer for training-time augmentation.

    The mixer samples a target SNR, selects one or more noise types, draws noise
    segments from the prebuilt library, scales each component to its target
    power, and adds the combined noise to the clean segment.

    WGN uses the same component-SNR floor policy as the offline test-data
    generation pipeline.
    """
    ...
    def __init__(
        ...
        self.cfg = config or {}
        self.noise_types = noise_types or self.cfg.get("noise", {}).get(
            "types",
            self.NOISE_TYPES,
        )
        ...
        self.snr_dist = str(
            ncfg.get("snr_train", {}).get("distribution", "uniform")
        ).lower()
        self.snr_test_grid = list(
            ncfg.get("snr_test", {}).get("grid", [-15, -10, -5, 0, 5, 10, 15])
        )
    ...
    def _load_paths(self):
        ...
        if not self.noise_paths:
            raise ValueError(f"No noise under {self.noise_root}")
    ...
    def _cache_all(self):
        ...
        print(f"  WGN floor: {WGN_COMPONENT_SNR_MIN_DB} dB component SNR")
    ...
    def mix(
        ...
        if seed is not None:
            su = int(seed) & 0xffffffff
            rng = random.Random(su)
            rnp = np.random.default_rng(su)
        else:
        ...
        max_k = min(self.k_max, len(available))
        min_k = max(1, min(self.k_min, max_k))
        k = rng.randint(min_k, max_k) if k is None else max(min_k, min(int(k), max_k))

        if mode == "train":
            snr_val = (
                float(rnp.uniform(self.snr_train_min, self.snr_train_max))
                if snr is None
                else float(snr)
            )
            ...
            snr_val = (
                float(rng.choice(self.snr_test_grid))
                if snr is None
                else float(snr)
            )
        ...
        if noise_types is not None:
            ...
            if len(selected) < k:
                selected += rng.sample(rest, min(k - len(selected), len(rest)))
        ...
        wgn_capped = (
            has_wgn
            and k >= 2
            and (snr_val + 10.0 * np.log10(k)) < WGN_COMPONENT_SNR_MIN_DB
        )
...
class OnlineMixingDataset:
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            return self.transform(nn), self.transform(cn)
...
def main():
    parser = argparse.ArgumentParser(description="sEMG noise generation and mixing")
    ...
    if args.demo:
        noise_root = os.path.join(
            get_output_base(config),
            config["paths"]["output"]["noise_train"],
        )

        if not os.path.isdir(noise_root):
            print("[ERROR] Run noise library generation first")
            return
        ...
        for i in range(4):
            ...
            wgn_note = (
                f"WGN component={comp:+.1f} dB {cap}"
                if comp is not None
                else "no WGN"
            )
            print(
                f"  SNR={info['snr']:+.1f} dB  "
                f"k={info['k']}  {wgn_note}  types={info['noise_types']}"
            )
        ...
        for snr in [-15, -10, -5]:
            ...
            cap = "WGN capped at floor" if info.get("wgn_capped") else "equal split"
            print(
                f"  total={snr:+d} dB  "
                f"WGN component={comp:+.1f} dB  {cap}"
            )
    ...
    if args.mode in ("train", "both"):
        build_noise_library(config, "train")

    if args.mode in ("test", "both"):
        build_noise_library(config, "test")
