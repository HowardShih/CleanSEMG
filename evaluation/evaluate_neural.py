...
Synced metric set with inference.py v6.7.0:
  SNRimp, RMSE, PRD, LSD, RMSE_ARV, RMSE_ZCR, RMSE_MNF, RMSE_MDF, RMSE_Kurtosis

Key difference from inference.py:
  - Models are waveform-based: noisy [B, L] → pred [B, L] (no STFT)
  - No OLA needed (models directly accept 2000-sample inputs)

Output: outputs/inference_baseline/{model_name}/
"""
...
def cal_SNR(clean: np.ndarray, test: np.ndarray) -> float:
    ...
    noise_pw  = np.dot(test - clean, test - clean)
    signal_pw = np.dot(clean, clean)
    if noise_pw < 1e-12:
        return 100.0
    return float(10.0 * np.log10(signal_pw / (noise_pw + 1e-12)))
...
def cal_LSD(clean: np.ndarray, enhanced: np.ndarray,
    ...
    LSD = mean_frames{ sqrt( mean_freq[ (10·log10 P_clean - 10·log10 P_enh)^2 ] ) }
    Ref: Wikipedia – Log-spectral distance; Rabiner & Juang 1993.
    """
    clean    = clean.reshape(-1).astype(np.float64)
...
def cal_RMSE_ZCR(clean, enhanced,
                 window_size: int = 200, sr: int = 1000) -> float:
    """
    RMSE of Zero-Crossing Rate (crossings/s) over non-overlapping windows.
    """
...
def _mnf_per_window(emg: np.ndarray,
                    sr: int = 1000,
                    f_min: float = 20.0, f_max: float = 500.0) -> np.ndarray:
    """
    Mean Frequency (MNF) per non-overlapping 200-ms window.
    Uses boxcar + STFT, identical to TrustEMG-Net convention (Wang et al. JBHI 2025).
    Ref: Phinyomark et al., IntechOpen, 2012.
    """
...
def cal_RMSE_MNF(clean, enhanced,
                 sr: int = 1000,
                 f_min: float = 20.0, f_max: float = 500.0) -> float:
    """
    RMSE of Mean Frequency (MNF) [Hz].
    Ref: Wang et al. JBHI 2025; Phinyomark et al. IntechOpen 2012.
    """
    try:
...
def _mdf_per_window(x: np.ndarray, window_size: int = 200,
    ...
    """
    Median Frequency (MDF) per non-overlapping window.
    MDF = freq where cumulative PSD = 50% total.
    Ref: Phinyomark et al. IntechOpen 2012; MATLAB medfreq().
    """
    x = x.reshape(-1).astype(np.float64)
...
def cal_RMSE_MDF(clean, enhanced, window_size: int = 200,
                 sr: int = 1000,
                 f_min: float = 20.0, f_max: float = 500.0) -> float:
    """
    RMSE of Median Frequency (MDF) [Hz].
    Ref: Phinyomark et al. IntechOpen 2012.
    """
    try:
...
def _kurtosis_per_window(x: np.ndarray,
                         window_size: int = 200) -> np.ndarray:
    """Fisher's excess kurtosis per non-overlapping window."""
    x = x.reshape(-1).astype(np.float64)
    out = []
    for i in range(0, len(x) - window_size + 1, window_size):
        seg = x[i:i + window_size]
...
def cal_RMSE_Kurtosis(clean, enhanced, window_size: int = 200) -> float:
    """
    RMSE of Fisher's excess kurtosis over non-overlapping windows.
    """
...
def calculate_all_metrics(
    ...
    Feature – freq : RMSE_MNF (MeanFreq), RMSE_MDF (MedianFreq)
...
def make_snr_k_table(col: ResultsCollector, metric: str,
    ...
    for d in col.results:
        ...
        for s in col.results[d]:
            snrs.add(s); ks.update(col.results[d][s].keys())
    rows = []
...
def make_snr_noisetype_table(col: ResultsCollector,
                              metric: str) -> pd.DataFrame:
    all_lbl    = set(col.all_noise_type_labels())
    snr_inputs = col.all_snr_inputs()
    ordered    = [l for l in _SINGLE_TYPE_ORDER + _KCOUNT_ORDER if l in all_lbl]
    ordered   += [l for l in sorted(all_lbl) if l not in ordered]
    rows = []
...
def run_inference(
    ...
    if not has_noise_paths:
        print("  [WARN] noise_paths missing — Pink/Brown subtypes unavailable.")
    ...
    with torch.inference_mode():
        for bi in tqdm(range(n_batches), desc=f"Inference [{model_name}]"):
            ...
            metas   = [{
                ...
                "snr":         item["snr"],
                "k":           item["k"],
                "noise_types": str(item.get("noise_types", "")),
            ...
            for i, meta in enumerate(metas):
                ...
                c_s = clean_np[i] * scale
                p_s = pred_np[i]  * scale
                n_s = noisy_np[i] * scale
                ...
                nt = _noise_type_labels(
                    meta["noise_types"],
                    meta["noise_paths"] if has_noise_paths else "",
                )
                collector.add(db=meta["dataset"], snr=meta["snr"],
                              k=meta["k"], metrics=m, noise_type_labels=nt)
...
# ============================================================================
def main():
    ap = argparse.ArgumentParser(description="sEMG Baseline Waveform Inference v2.0")
    ...
    ap.add_argument("--sr",        type=int, default=1000)
    args = ap.parse_args()
    ...
    run_inference(
        ...
        model_path=args.ckpt, test_data_path=args.test_data,
        output_dir=args.output, batch_size=args.batch,
        metrics_list=metrics, sampling_rate=args.sr,
    )
